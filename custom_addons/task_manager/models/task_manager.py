...
class TaskManager(models.Model):
    ...
    # Might be sensible to have additional Days Left as a computed
    # field to emphasize remaining days - friendlier UI
    # Not leaving it here since it is out of scope,
    # but it was useful for the testing

    def get_days_left(self):
        """
        Compute the number of days left until the due date.
        :return: Optional[int|None] Number of days until due date
        or None if due date is not set.
        """
        if self.due_date:
    ...
    def cron_notify_upcoming_tasks(self):
        """
        Method called by the cron job to notify users about
        tasks that are due in 3 days. It searches for tasks that are due
        in 3 days and have not been completed.
        ...
        for task in tasks:
            ...
            if days_left == 3 and task.assigned_to.email:
                # Could be a good idea to use a template for the email
                # body, but for sake of simplicity this is string
                body = f"""
                     <p>Hello {task.assigned_to.name},</p>
                     <p>This is a reminder that the task 
                ...
                try:
                    ...
                    # perform more actions than
                    # just sending an email (chat, etc.)
                    ...
                    mail.send()
                    # Log email sending
                    _logger.info(f"Sent task reminder to "
                                 f"{task.assigned_to.email}")
                except Exception as e:
                    _logger.error(f"Failed to send reminder "
                                  f"for task {task.name}: {e}")

    def make_tasks_report(self):
        """
        Generate a report for all tasks.
        This method retrieves all tasks and generates
        a report using the report action.
        ...
        return self.env.ref(
            'task_manager.action_report_task'
        ).report_action(tasks)

    def action_save_and_return(self):
        """
        Save the current task and return to the previous view.
        Allows Save and Close task button to avoid
        creating unknown breadcrumbs during click.
